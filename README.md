...
### 🎯 참고
본 프로젝트는 OTT 서비스 데이터를 기반으로 한 데이터 집계 및 분석용 Oracle DB 설계 실습입니다.
상용 수준의 Spark나 Oracle EE 환경은 비용과 리소스 제약으로 인해 직접 구현이 어려웠으므로,
주어진 여건에서 가능한 최선의 환경인 AWS EC2와 Oracle XE를 활용하여
DB 엔지니어링의 핵심 과정을 간단히 구현하고 실습하는 것을 목표로 하였습니다.

이를 통해 EC2 네트워크 구성, DB 설치 및 접속 설정, 데이터 모델링,
SQL 기반 집계 로직 설계 등 현실적인 제약 속에서도 가능한 한 실무에 가까운 환경을 구축하였습니다.
