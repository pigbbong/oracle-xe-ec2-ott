...
- 내부 테이블로 데이터를 적재하는 과정에서, 로컬 PC 환경과 달리 EC2 환경에서는 데이터 품질 관리에 여러 어려움이 발생하였음.  
  이를 통해 **OS 환경별로 데이터 인코딩 및 공백 처리 방식 등이 상이할 수 있음**을 확인하였으며,  
  데이터 정제와 품질 관리의 중요성을 실무적으로 체감함.  
  향후 유사한 환경에서는 이러한 차이를 고려하여 **데이터 표준화 및 정제 절차를 보다 체계적으로 설계**할 필요성을 인식함.
## 한계
